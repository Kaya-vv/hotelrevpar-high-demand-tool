...
const state = vi.hoisted(() => ({ tables: {} as Record<string, Record<string, unknown>[]>, fail: "" }));
const publish = vi.hoisted(() => vi.fn());
vi.mock("@/features/collection/jobs", () => ({ publishCollectionJob: publish }));
vi.mock("./query", () => ({ loadVisibleNotificationEvents: async () => ({ id: "hotel", name: "Hotel", events: [{ id: "event", title: "Festival", startAt: "2027-06-01", endAt: "2027-06-02", level: "High", venue: "Venue" }] }) }));
