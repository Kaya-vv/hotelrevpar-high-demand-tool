...
export function verifyEventEvidence(
  ...
  const citywideSupported = /city.?wide|across (?:the )?city|throughout (?:the )?city|door heel|in heel|verspreid|meerdere locaties|verschillende locaties|\d+ (?:locations|locaties)|multi.venue/i.test(facts.locationText);
...
export function supportedAudience(evidence: EventEvidence | undefined, audience: EventCandidate["overnightAudience"]) {
  ...
  return evidence.demand.some((fact) => /visitor|bezoek|audience|publiek|attend|delegate|deelnemer|liefhebber|enthusiast|music lover|\bfans\b/i.test(fact.text)
    && (audience === "international"
      ? /international|internationa|countries|landen|abroad|buitenland/i.test(fact.text)
