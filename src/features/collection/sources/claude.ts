...
async function collectClaudeFresh(
    ...
    try {
      ...
      return parsed.events.flatMap((event) => {
        ...
        if (event.ownerType === "other") {
          ...
          if (entry.title === null) invalidatedUrls.push(sourceUrl);
          return [];
        }
  ...
  return {
    ...
    funnel: {
      ...
      urlsResolved,
      pagesVerified,
      demandAccepted: candidates.filter((candidate) => hasHotelDemand(assessHotelDemand(candidate))).length,
      drops,
    },
  };
}
