...
it("runs the near-term search again after fourteen calendar days", () => {
  const now = new Date("2026-09-01T12:00:00Z");
  expect(claudeDiscoveryDue(null, now)).toBe(true);
  expect(claudeDiscoveryDue("2026-08-19T00:00:00Z", now)).toBe(false);
  expect(claudeDiscoveryDue("2026-08-18T23:59:59Z", now)).toBe(true);
});
...
describe("runCollection", () => {
  ...
  it("runs scheduled Claude discovery at most once every 14 days", async () => {
    const claude = vi.fn();
    const repo = repository({
      shouldRunClaudeDiscovery: vi.fn().mockResolvedValue(false),
      loadContext: vi.fn().mockResolvedValue({
    ...
    expect(result.sourceResults.claude).toEqual({
      ...
      reason: "Claude discovery runs at most once every 14 days per market.",
    });
  });
