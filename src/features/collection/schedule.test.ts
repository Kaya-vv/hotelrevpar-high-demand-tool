...
it("starts a new hotel immediately and makes both recurring and broad searches due after 30 days", () => {
  ...
  expect(searchDue(first, new Date("2026-10-05T05:00:00Z"))).toBe(false);
  expect(searchDue(first, new Date("2026-10-05T05:00:00Z"), BROAD_SEARCH_INTERVAL_DAYS)).toBe(false);
  expect(searchDue(first, new Date("2026-10-20T23:59:59Z"))).toBe(false);
  expect(searchDue(first, new Date("2026-10-21T05:00:00Z"))).toBe(true);
  expect(searchDue(first, new Date("2026-10-20T23:59:59Z"), BROAD_SEARCH_INTERVAL_DAYS)).toBe(false);
  expect(searchDue(first, new Date("2026-10-21T05:00:00Z"), BROAD_SEARCH_INTERVAL_DAYS)).toBe(true);
});
...
it("counts a recent manual refresh and crosses month and year boundaries", () => {
  ...
  expect(searchDue("2026-12-28T21:00:00Z", new Date("2027-01-26T05:00:00Z"))).toBe(false);
  expect(searchDue("2026-12-28T21:00:00Z", new Date("2027-01-27T05:00:00Z"))).toBe(true);
});
