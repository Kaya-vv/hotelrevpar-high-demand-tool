...
import { coveringMarket, longRangeMarketKey, type LongRangeState } from "./long-range-store";
...
export const MARKET_STATUS_COLUMNS =
  "market_key, search_location, radius_km, progressPublishedAt:state->>progressPublishedAt, publishedAt:state->>publishedAt, publicationPending:state->publicationPending, research:state->research, waves:state->cycle->waves, leadKeys:state->cycle->leadKeys";
...
export async function getMarketProgress(keys: string[]): Promise<Map<string, Pick<LongRangeState, "publishedAt" | "progressPublishedAt" | "publicationPending" | "research">>> {
  const rows = await fetchInBatches(keys, (ids) =>
    createAdminClient()
      .from("long_range_markets")
      .select(
        "market_key, search_location, radius_km, progressPublishedAt:state->>progressPublishedAt, publishedAt:state->>publishedAt, publicationPending:state->publicationPending, requestedAt:state->research->>requestedAt",
      )
      .in("market_key", ids),
  );
  return new Map(
    rows.map((row) => [
      row.market_key,
      {
        ...
        progressPublishedAt: row.progressPublishedAt ?? undefined,
