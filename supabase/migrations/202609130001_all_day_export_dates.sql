-- An all-day record is stored with a UTC end-of-day placeholder (`...T23:59:59+00:00`). The
-- application reads that placeholder as the day it names (`eventLocalDate`, see
-- src/features/events/normalize.ts), so the workbook prints e.g. IBC2026 as ending 14 September.
-- This transaction re-derived the same dates by converting the instant to Europe/Amsterdam, which
-- lands on 15 September, so the guard declared the event "changed during generation" and rejected
-- the whole batch. Every selection containing such an event answered 409 and could never be
-- exported. The claude collector still writes these placeholders, so both sides must read them
-- identically rather than the data being repaired once.
create or replace function public.event_local_date(p_at timestamptz) returns text
language sql stable set search_path = public as $$
  select case
    when date_trunc('second', p_at) = p_at and (p_at at time zone 'UTC')::time = time '23:59:59'
      then to_char(p_at at time zone 'UTC', 'YYYY-MM-DD')
    else to_char(p_at at time zone 'Europe/Amsterdam', 'YYYY-MM-DD')
  end;
$$;
revoke all on function public.event_local_date(timestamptz) from public, anon, authenticated;
  ...
  for item in select value from jsonb_array_elements(p_items) loop
    ...
    if not exists(
      ...
      where e.id=(item->>'eventId')::uuid and e.certainty='confirmed' and e.source_state='active'
        ...
        and event_local_date(coalesce(ae.override_start_at,e.start_at))=item->>'startDate'
        and event_local_date(coalesce(ae.override_end_at,e.end_at))=item->>'endDate'
