-- A hand-set demand level (`hotel_event_scores.importance_override`) is a decision, not a guess,
-- so it publishes on its own authority. `impact_basis = 'default'` only records that the automatic
-- scorer found nothing to go on, which must not veto the person who graded the event by hand.
-- Without this, an event graded High in the calendar reached the export screen but this guard
-- rejected the whole batch with "Export event changed during generation" (HTTP 409).
  ...
  for item in select value from jsonb_array_elements(p_items) loop
    ...
    if not exists(
      ...
      where e.id=(item->>'eventId')::uuid and e.certainty='confirmed' and e.source_state='active'
        ...
        and ((item->>'manual')::boolean
          or ((hs.importance_override is not null or hs.impact_basis <> 'default')
