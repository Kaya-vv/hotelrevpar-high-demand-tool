-- Existing scores remain unassessed until replay/recalculation from source evidence.
alter table public.hotel_event_scores add column demand_assessment jsonb;

-- Recheck the same demand policy when committing the workbook.
  ...
  for item in select value from jsonb_array_elements(p_items) loop
    ...
    if not exists(
      ...
      where e.id=(item->>'eventId')::uuid and e.certainty='confirmed' and e.source_state='active'
        ...
        and hs.demand_assessment->>'relevance' in ('supported','probable')
        and ((item->>'manual')::boolean or (hs.impact_basis='demand_rule'
